from f10_reference import Sig, Chain, Witness, verify_receipt, canon, GENESIS_SEED
...
class GossipClient:
    ...
    def _verify(self, r, witness_pub, writer_registry):
        '''Verify writer sig over checkpoint, then witness sig over full signed writer-envelope.'''
        Sig('v', GENESIS_SEED).verify(canon(r['checkpoint']), r['writer_sig'],
                                     writer_registry[r['writer_key_id']])
        verify_receipt(r, witness_pub)
    def collect(self, chain_id, event_id, roster, writer_registry):
        for w, pub in roster:
            ...
            if rs:
                for r in rs:
                    self._verify(r, pub, writer_registry)
                self.merge(rs)
    def merge(self, receipts):
        for r in receipts:
            ...
            if not any(canon(x) == canon(r) for x in bucket):
                bucket.append(r)
    def conflict_report(self, chain_id, event_id, witness_pubs, writer_registry):
        '''Returns report; NEVER claims absence of conflict elsewhere.'''
        rs = self.receipts.get((chain_id, event_id), [])
        for r in rs:
            self._verify(r, witness_pubs[r['witness_key_id']], writer_registry)
