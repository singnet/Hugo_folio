...
class Chain:
    ...
    def execution_intent(self, operation_id, tool, input_blob):
        return self._emit({"type": "execution_intent", "operation_id": operation_id,
                           "tool": tool, "input_hash": h(input_blob)})

    def execution_result(self, operation_id, output_blob):
        return self._emit({"type": "execution_result", "operation_id": operation_id,
                           "output_hash": h(output_blob)})
...
class Witness:
    ...
    def receive(self, checkpoint_msg):
...
def verify_chain(events, registry):
    head = h(GENESIS_SEED)
    for ev in events:
...
class MockTool:
    def __init__(self):
        self.done = set()
    def call(self, chain, operation_id, arg):
        ...
        out = b"result:" + arg
        chain.execution_result(operation_id, out)
